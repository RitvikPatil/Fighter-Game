...
const updateGame = (p1,p2,gameState) => {
  ...
  if(p1.health <= 0 || p2.health<=0){
    game.isOver = true;
    gameState = true;
    const result = game.declareWinner(game.isOver, p1, p2);
    console.log(result)
...
class Player {
  ...
  heal (player) {
    ...
    const healAmount = Math.floor(Math.random() * 5);
    ...
    resultDiv.innerText = `${player.name} heals for ${healAmount} HP`;
...
let p1 = new Player('Ritvik', 100, 15);
let p2 = new Player('Soham', 100, 15);
